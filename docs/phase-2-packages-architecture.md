...
当前生产入口为 `packages/core`、`packages/analyzer`、`packages/devtools`、`packages/vite`、
`packages/rsbuild` 和 `packages/webpack`；`packages/css-loader-bridge` 是 Rsbuild/Webpack 内部共享 seam。真实构建工具
消费方验收分别收敛到 `fixtures/vite-css-modules`、`fixtures/rsbuild-css-modules` 与
`fixtures/webpack-css-modules` workspace，fixture 不是生产 adapter 包；三个对应 `playground/*` 分别承担
中型人工 Pilot，不进入自动门禁。

adapter 依赖方向为 `vite -> core/analyzer` 与 `rsbuild/webpack -> css-loader-bridge -> core/analyzer`，
adapter 之间互不依赖。Vite 使用其 CSS plugin seam 和独立 production serializer；Rsbuild/Webpack 共享
css-loader rows/default-locals 的纯转换、readable-keyed 命名、token/CSS 闭合校验、稳定聚合与 browser owner，但分别保留 Rspack/Webpack loader
request、compiler、HTML 和 dev-server 生命周期。Rsbuild 使用公开 loader `importModule`、css-loader array export 和 Rspack asset hooks。Rsbuild build 保持
原生 extraction；dev 切换到官方 style injection 维持模块图，并将目标模块快照注册到单一共享 style
owner，按 canonical source-id 顺序输出且按 atomic key 去重。这既避免 Rspack 2.1 extraction 的嵌套
`importModule` 增量编译 panic，也建立不依赖业务 import 顺序的全局 atomic cascade；跨模块同权重冲突
不属于 semantic/native parity 契约。Webpack build 使用
MiniCssExtractPlugin + HtmlWebpackPlugin，dev 使用 style-loader + webpack-dev-server，cache metadata 通过
loader emitFile/processAssets 在跨进程 filesystem cache 中重放。三个 adapter 都不复制 CSS Modules、
ICSS、预处理器或资源编译实现。
...
Playwright 加入生产依赖；也不读取文件或分析 CSS。三个 adapter 依赖 devtools protocol/runtime，各自使用
原生 middleware/HTML hook 暴露当前状态，devtools 不反向依赖 adapter。

dev report 复用 core report 与 analyzer analysis，envelope 直接以 `adapter`、`status`、`environments`
（失败时另带 `error`）区分状态，当前不含 `schemaVersion`；现有 build report schema 不变。完整 CSS source map 仍需要 adapter 上游 map 与 core generated mapping 组合，不属于 devtools
